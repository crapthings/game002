...
export function prepareCommitment(world,{kind,actorId,opportunityId,terms},context) {
  ...
  if(row.assigneeId===actorId) {
    if(kind==='collect'&&row.status==='fulfilled')receiptId=row.completionEventId
  ...
  const commandKind={accept:'accept',decline:'decline',cancel:'cancel',deliver:'deliver_message',collect:'collect_reward'}[kind]
