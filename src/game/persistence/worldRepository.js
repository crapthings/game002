...
const ACTIVE_KEY = 'game002:active-seed:512:v10'
...
export function validateDocument(document, seed) {
  if (![SCHEMA_VERSION,3,4,5].includes(document?.schemaVersion) || !Number.isInteger(document.revision) || document.revision < 0) throw new Error('存档版本无效，已保留原始数据。')
...
export function createWorldRepository(storage) {
  ...
  return {
    ...
    save(document, progress) {
      ...
      const next = { ...document, schemaVersion:progress.living?.version===3?5:progress.living?.version===2?4:progress.living?3:document.schemaVersion, revision: document.revision + 1, progress }
