...
function database() {
  if (!connection) connection = new Promise((resolve, reject) => {
    const request = indexedDB.open('game002-worlds-512-v6', 1)
...
export const asyncWorldRepository = {
  async open(value) {
    ...
    if (!document) {
      ...
      const raw = localStorage.getItem('game002:world:512:v6:' + encodeURIComponent(seed))
    ...
    localStorage.setItem('game002:active-seed:512:v6', seed)
    return document
  },
