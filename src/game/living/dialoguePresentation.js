...
export function spokenAnswer(world,answer,names) {
  ...
  if(answer.kind==='requests')return [...(answer.text?[answer.text]:[]),...(answer.opportunityIds.length?['这些是我想请人帮忙、或之前提过的事情；进展以我亲历和听到的消息为准。',...answer.opportunityIds.map(id=>world.opportunities.entries.find(r=>r.id===id)?.title).filter(Boolean)]:['眼下没有已经登记、可请你承接的请求。'])]
  return [answer.text]
}
