...
export default function MapCanvas({ center, span, navigation, plan, radar = false, onPointerDown, onPointerMove, onPointerUp, onWheel }) {
  ...
  const markers=useLivingStore(state=>state.view?.places),trackedPlaceId=useLivingStore(state=>state.trackedPlaceId)
  useEffect(() => {
    ...
    const render = () => {
      ...
      drawMap(context, rect.width, rect.height, { center, span, ...navigation, plan, radar, revealMap, markers, trackedPlaceId })
    }
    render()
    const observer = new ResizeObserver(render)
    observer.observe(canvas)
    return () => observer.disconnect()
  }, [center, span, navigation, plan, radar, revealMap, markers, trackedPlaceId])
