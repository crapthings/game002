...
export default function LivingHud(){
 ...
 const v=useLivingStore(s=>s.view),panel=useLivingStore(s=>s.panel),message=useLivingStore(s=>s.message),attention=useLivingStore(s=>s.attention),request=useLivingStore.getState().request
 const formatMessage=text=>SERVICE_REASONS[text]??errors[text]??(/^[A-Z_]+$/.test(text)?'当前条件不满足，暂时无法执行。':text)
 ...
 return <>
  ...
  <aside className="absolute bottom-4 left-1/2 w-80 max-w-[90vw] -translate-x-1/2 rounded-xl border border-white/20 bg-stone-950/90 p-3 text-xs text-stone-100" aria-label="战斗状态">
   ...
   {message&&<div className="mt-2 text-amber-200"><p aria-live="polite">{formatMessage(message)}</p>{attention.current&&<button className="mt-1 text-stone-400 underline" onClick={()=>useLivingStore.getState().dismissNotice(attention.current.id)}>收起</button>}</div>}
  </aside>
  {panel&&<section aria-label="背包与交互" className="absolute left-4 top-28 z-20 max-h-[70vh] w-96 max-w-[92vw] overflow-y-auto rounded-xl border border-white/20 bg-stone-950/95 p-4 text-sm text-stone-100">
   ...
   <AttentionPanel view={v} formatMessage={formatMessage} onReview={showReview}/>
