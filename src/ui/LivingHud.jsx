...
export default function LivingHud(){
 ...
 const events=useMemo(()=>{
  ...
  const s=v.state
  return [...s.combat.events,...s.interactions.events,...s.village.events,...s.robbery.events,...s.property.events,...s.equipment.events,...s.crime.events,...(s.life?.events??[]),...(s.places?.events??[]),...s.social.events.filter(e=>e.kind==='report'||e.kind==='witness')].sort((a,b)=>a.at-b.at)
 ...
 return <>
  ...
  {panel&&<section aria-label="背包与交互" className="absolute left-4 top-28 z-20 max-h-[70vh] w-96 max-w-[92vw] overflow-y-auto rounded-xl border border-white/20 bg-stone-950/95 p-4 text-sm text-stone-100">
   ...
   <details className="mt-4" open={reviewOpen} onToggle={e=>setReviewOpen(e.currentTarget.open)}><summary>事后因果回顾</summary><p className="my-2 text-xs text-stone-400">这里展示世界事实；附近看不到的案件和人物状态，不会成为地图上的实时追踪。</p>{events.slice(-40).map(e=><p className="my-2 text-xs" key={e.id}>{(e.at/1000).toFixed(1)}秒 · {lifeWords[e.kind]??words[e.kind]??'记录变化'} · {name(e.actorId??e.authorityId)}{e.targetId?` → ${name(e.targetId)}`:''}{e.placeId?` · ${s.places?.definitions.find(p=>p.id===e.placeId)?.label??'原定场所'}`:''}{e.damage?` -${e.damage}气血`:''}{e.amount?` ${e.amount}文`:''}{e.subjectId===null?' · 身份不明':''}<br/><span className="text-stone-500">{e.cause?`源于：${words[events.find(source=>source.id===e.cause)?.kind]??'已记录的事件'}`:''}</span></p>)}</details>
