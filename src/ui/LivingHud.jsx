...
const opportunityWords={opportunities_initialized:'登记生活请求',opportunity_offered:'提出委托',opportunity_disclosed:'告知请求',opportunity_accepted:'答应帮忙',opportunity_declined:'婉拒请求',opportunity_cancelled:'取消委托',opportunity_expired:'委托到期',opportunity_failed:'委托未能完成',opportunity_funds_lost:'报酬资金受损',message_delivered:'口信送达',message_acknowledged:'收信人答复',opportunity_payment_due:'约定报酬待付',opportunity_fulfilled:'办妥并结清'}
...
export default function LivingHud(){
 ...
 const events=useMemo(()=>{
  ...
  const s=v.state
  return [...s.combat.events,...s.interactions.events,...s.village.events,...s.robbery.events,...s.property.events,...s.equipment.events,...s.crime.events,...(s.life?.events??[]),...(s.places?.events??[]),...(s.dialogue?.events??[]),...(s.opportunities?.events??[]),...s.social.events.filter(e=>e.kind==='report'||e.kind==='witness')].sort((a,b)=>a.at-b.at)
 ...
 return <>
  ...
  {panel&&<section aria-label="背包与交互" className="absolute left-4 top-28 z-20 max-h-[70vh] w-96 max-w-[92vw] overflow-y-auto rounded-xl border border-white/20 bg-stone-950/95 p-4 text-sm text-stone-100">
   ...
   <details className="mt-4" open={reviewOpen} onToggle={e=>setReviewOpen(e.currentTarget.open)}><summary>事后因果回顾</summary><p className="my-2 text-xs text-stone-400">这里展示世界事实；附近看不到的案件和人物状态，不会成为地图上的实时追踪。</p>{events.slice(-40).map(e=><p className="my-2 text-xs" key={e.id}>{(e.at/1000).toFixed(1)}秒 · {opportunityWords[e.kind]??dialogueWords[e.kind]??lifeWords[e.kind]??words[e.kind]??'记录变化'} · {name(e.actorId??e.authorityId)}{e.targetId?` → ${name(e.targetId)}`:''}{e.placeId?` · ${s.places?.definitions.find(p=>p.id===e.placeId)?.label??'原定场所'}`:''}{e.damage?` -${e.damage}气血`:''}{e.amount?` ${e.amount}文`:''}{e.subjectId===null?' · 身份不明':''}<br/><span className="text-stone-500">{e.cause?`源于：${opportunityWords[events.find(source=>source.id===e.cause)?.kind]??words[events.find(source=>source.id===e.cause)?.kind]??'已记录的事件'}`:''}</span></p>)}</details>
   <details className="mt-4" open={knowledgeOpen} onToggle={e=>setKnowledgeOpen(e.currentTarget.open)}><summary>角色各自知道什么</summary>{knowledgeOpen&&Object.entries(v.names).map(([id,n])=><div key={id} className="mt-2 text-xs"><strong>{n}</strong>{s.social.knowledge.filter(k=>k.npcId===id).map(k=><p key={k.factId}>{messageKnowledge(s,k,v.names)??`${words[s.social.facts.find(f=>f.id===k.factId)?.action]??'事件'} · ${k.subjectId?name(k.subjectId):'身份不明'} · ${s.social.events.find(e=>e.id===k.evidenceId)?.kind==='report'?'当面获知':'亲眼目击'}`}</p>)}</div>)}</details>
