...
export const useLivingStore=create((set,get)=>({
  view:null,message:'',panel:false,commands:[],flush:async()=>true,
  setFlush:flush=>set({flush}),
  shift:()=>{const [command,...rest]=get().commands;set({commands:rest});return command},
  ...
  clear:()=>set({commands:[]}),reset:()=>set({view:null,message:'',panel:false,commands:[],flush:async()=>true}),
}))
