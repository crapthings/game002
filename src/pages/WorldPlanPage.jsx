...
export default function WorldPlanPage() {
  ...
  const [layers,setLayers]=useState({macros:false,districts:false,parcels:false,regions:false,details:false,settlements:true,roads:true,labels:true})
  ...
  return <main className="flex h-dvh min-h-96 flex-col bg-[#0c1314] text-stone-200">
    ...
    <div className="flex shrink-0 flex-wrap items-center gap-4 border-b border-white/10 px-4 py-2 text-xs">
      ...
      {Object.entries({macros:'2×2 格局',districts:'4×4 街坊',details:'8×8 街区',parcels:'16×16 地块',labels:'名称'}).map(([key,label])=><label key={key} className="flex items-center gap-1.5"><input type="checkbox" checked={layers[key]} onChange={event=>setLayers(value=>({...value,[key]:event.target.checked}))}/>{label}</label>)}
    ...
    <div className="grid min-h-0 flex-1 grid-rows-[minmax(240px,1fr)_180px] md:grid-cols-[minmax(0,1fr)_250px] md:grid-rows-1">
      ...
      <aside className="min-h-0 overflow-y-auto border-t border-white/10 bg-[#111b1a] p-4 md:border-l md:border-t-0">
        ...
        {plan?.city && <div className="mt-3 text-xs leading-6 text-emerald-200"><p>{plan.city.placements.filter(p=>p.building).length} 栋建筑 · {plan.city.parcels.length} 个城内地块</p><p>{plan.city.roads.length} 条街巷 · {plan.city.bridges.length} 座桥 · 碧溪水系</p><p>4 / 16 / 64 / 256 个层级单元</p></div>}
        ...
        <p className="mt-2 text-xs leading-6 text-stone-400">{region?`${Math.round(area)} m² · 占全图 ${(area/(plan.size*plan.size)*100).toFixed(1)}%`:'0.262144 km² · 城内街坊与城外自然区域'}<br/>{town?`${town.placements.length} 栋建筑 · ${town.blocks?.length || 0} 个街区`:region?'自然生态区域':'城内街坊、民居与水系'}</p>
        ...
        <p className="mt-4 text-[10px] leading-5 text-stone-500">2×2 定功能格局，4×4 分配街坊，8×8 连通街网，16×16 生成宅院和入户小路。城外保留自然生态。</p>
        ...
        <p className="mt-4 text-[10px] leading-5 text-stone-500">当前算法预览不读取或覆盖存档。旧版本保留原布局。</p>
      </aside>
    </div>
  </main>
}
