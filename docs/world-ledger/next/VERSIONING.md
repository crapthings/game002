...
设计基线：78cf8df（P0）。本页固定迁移规则，不是运行验收证明。新场景起步为 living v3 / document schema 5 / gameplay v2；12-B 首次归档后为 living v4 / schema 6，玩法 v2 内使用 archive v2。见 [分页与恢复](ARCHIVE-IMPLEMENTATION.md)。living v1、P0 v2 与未分页 v3 仍可恢复。
...
重复请求先查活跃日志，再查原 v1 日志及分页请求索引，内容一致返回既有成功，内容不同拒绝。12-B 保留原活跃窗口保护，页与回执索引保存旧回执信息，不扩大常数。
...
新增领域在 v2 增量日志中通过明确命令初始化，不改变迁移初始基线。旧 v1 日志及归档索引参与请求 ID 去重，`live-N` 编号也查旧档和归档最大值。12-B 已接实际分页保存路径，但仍没有容量和保存故障的运行验证。
